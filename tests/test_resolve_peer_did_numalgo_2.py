...
def test_resolve_numalgo_2_signing_invalid_key_type():
    with pytest.raises(
        MalformedPeerDIDError,
        match=r"Invalid peer DID provided.*Authentication not supported for key",
    ):
        resolve_peer_did(
...
def test_resolve_numalgo_2_encryption_invalid_key_type():
    with pytest.raises(
        MalformedPeerDIDError,
        match=r"Invalid peer DID provided.*Key agreement not supported for key",
    ):
        resolve_peer_did(
            "did:peer:2.Ez6LSbysY2xFMRpGMhb7tFTLMpeuPRaqaWM1yECx2AtzE3KCc"
